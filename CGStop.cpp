...
CGStop::CGStop(DSObject *father,
               DSString *label_name) :
  DSStop(father, label_name)
{
}
...
DSObject *CGStop::New(DSObject *father) const
{
  return (DSStop *)new CGStop(father);
}

/****************************************************************************
 * Write(): Oberfunktion der Codegenerierung fuer Stop
 *          -> writer
 *          -> what
 *          Ergebnis: DS_ OK oder DS_ERROR  
 *          Seiteneffekte: Anlegen und Fuellen der .h und .cc-Files
 ****************************************************************************/
 
DSResult CGStop::Write(DSWriter *writer, DSCardinal what) const
{
  DSResult result;
...
/****************************************************************************
 * WriteImplementation(): schreibt einen DSStop-Typ
 *                        -> writer
...
DSResult CGStop::WriteImplementation(CGWriter *writer) const
{
  DSResult result;
  DSString str;
  CGFile *out;
...
//  str.Format("%scur_state = %sstop;", PREFIX_VARIABLE, PREFIX_STATE_TYPE_ID);
//  InsertString(*out, str, pos, CG_WITH_EOL);
//  InsertString(*out, "goto nextstate;", pos, CG_WITH_EOL);

  InsertString(*out, "Stop();", pos, CG_WITH_EOL);
...
DSResult CGStop::WriteLabel(CGWriter *writer) const
{
  DSResult result;
  ...
  else // Doppeltes Label!
  {
    if (writer->CGVerbose())
    {
      writer->cgCout << "Duplicate label '" << GetLabel() << "'." << std::endl;
    }
  }
